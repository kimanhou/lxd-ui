...
const NetworkAclDetailHeader: FC<Props> = ({ name, networkAcl, project }) => {
  ...
  const notifySuccess = (aclName: string, url: string) => {
    toastNotify.success(
      <>
  ...
  const formik = useFormik<RenameHeaderValues>({
    ...
    onSubmit: (values) => {
      ...
      renameNetworkAcl(name, values.name, project)
        .then((operation) => {
          ...
          if (hasStorageAndNetworkOperations) {
            ...
            eventQueue.set(
              ...
              () => {
                notifySuccess(values.name, url);
              },
              (msg) =>
                toastNotify.failure(
                  `Renaming of network ACL ${values.name} failed`,
                  new Error(msg),
                ),
            );
          } else {
        ...
        .catch((e) => {
          notify.failure("Renaming  of network ACL failed", e);
        })
        .finally(() => {
          formik.setSubmitting(false);
        });
